...
class SitereportSpider(scrapy.Spider):
	...
	def start_requests(self):
		...
		if lastRequest is not None:
			...
			self.proxy_user_pass = lastRequest[4]
			found = True
		else :
	...
	def parse_requests(self, response):
		...
		if login == False:
			...
			if self.proxy_user_pass is not None:
				request.headers['Proxy-Authorization'] = self.get_proxy_authorization()
			return request
		pass
	...
	def get_proxy_authorization(self, user, password):
		basic_auth = 'Basic ' + base64.encodestring(self.proxy_user_pass)
		return basic_auth
	...
	def get_proxies(self):
		db = self.make_connection_MySQL()
		cursor = db.cursor()
		sql = "SELECT id, proxy, CONCAT(username,':',password) FROM Proxies"
		cursor.execute(sql);
		result = cursor.fetchall()
		db.close()
		...
		while count_account < len(accounts) and found is False:
			...
			while count_proxy < len(proxies) and found is False:
				...
				if result is None:
					...
					self.proxy_user_pass = proxies[count_proxy][2]
	...
	def get_available_account_proxy(self):
		db = self.make_connection_MySQL()
		cursor = db.cursor()
		sql = "SELECT a.username, a.password, ap.id, p.proxy, CONCAT(p.username,':',p.password) FROM SeoAccountProxies ap JOIN SeoToolAccounts a ON a.id = ap.seoAccountId JOIN Proxies p ON p.id = ap.proxyId WHERE NOW() > ap.updatedTime + INTERVAL 1 DAY OR ap.lastResponseStatus = 200 OR ap.used = 0"
		cursor.execute(sql)
		result = cursor.fetchone()
		db.close()
